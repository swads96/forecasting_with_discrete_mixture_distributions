...
<<echo=FALSE>>=
source('../code/makemixdist.R')
source('../code/crps.R')

preddf1 <- data.frame(family=c('Lnorm','Norm'),param1= c(2,2.1), param2=c(1,1),
                     param3=NA,weights=c(.3,.7))

preddf2 <- data.frame(family=c('Norm','Norm'),param1= c(1.5,4), param2=c(1,2),
                     param3=NA,weights=c(.4,.6))
ob <- 3
@
...
\ref{tab:preddf2} with scores of \Sexpr{round(-log(dmdist1(ob)),3)} and 
\Sexpr{round(-log(dmdist2(ob)),3)} respectively. 
However, under the CRPS forecast 2 outperforms forecast 1
with scores \Sexpr{round(CRPS(pmdist1,y=ob),3)} and 
\Sexpr{round(CRPS(pmdist1,y=ob),3)} respectively. We continue to use these same
...
component of mixture forecast 1 with weight \Sexpr{round(w1,3)}. The 
red line is the density component of mixture forecast 2 with weight 
\Sexpr{round(w2,3)}. The blue line is the density of the ensemble forecast.}
\label{fig:mixense}
\end{minipage}
\end{center}
\end{figure}
